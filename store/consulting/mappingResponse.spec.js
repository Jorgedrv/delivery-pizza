...
describe('Consulting manager test', () => {

    const actual = "0703601006";

	test('getConsulting reducers test', () => {
		const expected = mappingResponse.getConsulting(consulting);
		equal(actual, expected.rutcli);
	});

	test('getConsulting reducers test', () => {
		const expected = mappingResponse.getConsultDetails(consulting);
		equal(actual, expected.rutcli);
	});
